  ...
  return (
    <Stack w="100%">
      <List alignSelf={{base:"center", md: "end"}}>
        <RefreshButton border="1px" title="Refresh widgets" onClick={() => dispatch(fetchDevices())} />
      </List>
      <Box w="100%">
        <SimpleGrid w="100%" minChildWidth="16rem" spacing="2rem">
          {devices.array.map(device => {console.log("1"); return <Widget key={device.id} title={device.name} />})}
        </SimpleGrid>
      </Box>
    </Stack>
  );
